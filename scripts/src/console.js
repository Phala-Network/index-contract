...
function useChainEndpoint(config, chainName) {
  const chain = config.chains.find(
    (chain) => Object.keys(chain)[0] === chainName
  );
...
function useChainHandler(config, chainName) {
  const chain = config.handlers.find(
    (chain) => Object.keys(chain)[0] === chainName
  );
...
function useEvmHandler(config, chainName, key) {
  ...
  return new ethers.Contract(handlerAddress, HandlerABI, wallet);
}
...
async function useExecutor(api, pruntime_endpoint, contract_id) {
  const contract = loadContractFile(
    path.join(
      __dirname,
      '../../target/ink/index_executor/index_executor.contract'
    )
  );
...
program
  .option(
    '--config <path>',
    'config that contains contract and node informations',
    process.env.CONFIG || 'config.json'
  )
  .option(
    '--uri <URI>',
    'the account URI use to sign cert',
    process.env.URI || '//Alice'
  );

const keystore = program.command('keystore').description('inDEX keystore');
...
const executor = program.command('executor').description('inDEX executor');
...
executor
  .command('setup')
  .description(
    'setup executor, stuff contains 1) import worker key from KeyStore contract; 2) claim rollup storage; 3) setup worker account in rollup storage'
  )
  ...
  .action(
    run(async (opt) => {
      ...
      {
        // costs estimation
        let { gasRequired, storageDeposit } = await executor.query.config(
          cert,
          {},
          100,
      ...
      if (opt.resume !== false) {
        // costs estimation
        let { gasRequired, storageDeposit } =
          await executor.query.resumeExecutor(cert, {});
        // transaction / extrinct
        let options = {
          gasLimit: gasRequired.refTime,
          storageDepositLimit: storageDeposit.isCharge
...
executor
  ...
  .option(
    '--free',
    'list worker account that currently not been allocated',
    false
  )
  .action(
    run(async (opt) => {
...
const task = program.command('task').description('inDEX task inspector');
  ...
  .action(
    run(async (opt) => {
      ...
      if (useChainType(opt.chain.toLowerCase()) === 'Evm') {
        let handler = useEvmHandler(
          config,
          opt.chain.charAt(0).toUpperCase() + opt.chain.slice(1).toLowerCase(),
          opt.key
        );
        let tx = await handler.setWorker(opt.worker, {
          gasLimit: 2000000,
        });
...
handler
  ...
  .requiredOption(
    '--worker <worker>',
    'woker H160 address on EVM chain, sr25519 public key on substrate chain',
    null
  )
  .action(
    run(async (opt) => {
...
handler
  ...
  .requiredOption(
    '--asset <asset>',
    'asset H160 address on EVM chain, encoded MultiLocation on substrate chain',
    null
  )
  .action(
    run(async (opt) => {
    ...
    run(async (opt) => {
      ...
      if (opt.worker !== null) {
        console.log(
          workers.find(
            (worker) =>
              worker.account32.toLowerCase() === opt.worker.toLowerCase()
          )
        );
      } else {
...
worker
  ...
  .action(
    run(async (opt) => {
      ...
      console.log(
        `Call Executor::worker_approve to approve ERC20 for specific asset`
      );
...
worker
  ...
  .requiredOption(
    '--asset <asset>',
    'Asset location<smart contract address for encoded multilocation>',
    null
  )
      ...
      if (opt.chain) {
        ...
        if (useChainType(opt.chain.toLowerCase()) === 'Evm') {
          ...
          if (opt.asset === null) {
            ...
            throw new Error(
              `Not support balance query for asset: ${opt.asset}`
            );
          }
        }
      } else {
...
const erc20Client = program
  .command('erc20')
  .description('interact with erc20 contract');

erc20Client
  .command('approve')
  .description('approve ERC20 for specific asset')
  .requiredOption('--rpc <rpc>', 'endpoint', null)
  .requiredOption('--token <token>', 'ERC20 token contract address', null)
  .requiredOption('--spender <spender>', 'spender H160 address', null)
  .requiredOption('--amount <amount>', 'the amount set to allowance', null)
  .action(
    run(async (opt) => {
      ...
      const provide = new ethers.JsonRpcProvider(opt.rpc);
      const wallet = new ethers.Wallet(uri, provide);
      const tokenContract = new ethers.Contract(opt.token, ERC20ABI, wallet);
      const tx = await tokenContract.approve(opt.spender, opt.amount);
      console.log(`${tx}: approve ${opt.spender} to use up to ${opt.amount}`);
    })
  );
